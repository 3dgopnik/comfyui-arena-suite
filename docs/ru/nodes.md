...
Пакет включает фронтенд-расширение `web/extensions/arena_autocache.js`, которое автоматически подключается в ComfyUI и слушает сокеты `summary_json`/`warmup_json`/`trim_json`. При получении новых значений расширение:

- распознаёт структуру JSON и отображает статусную строку и детали из блока `ui`;
- рисует полосы прогресса для аудита, прогрева, трима и заполненности кэша;
- подсвечивает заголовок узла по уровню (`ok`/`warn`/`error`), используя цвета заголовка и фона;
- выводит предупреждения о пропущенных моделях, ошибках прогрева и проблемах парсинга.

Оверлей работает с `ArenaAutoCacheAudit`, `ArenaAutoCacheDashboard` и `ArenaAutoCacheOps` и не требует ручной настройки: достаточно открыть ComfyUI с установленным пакетом.
