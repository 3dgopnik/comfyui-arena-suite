...
> **Примечание.** Если модуль `legacy` не сможет импортироваться (например, из-за отсутствующих зависимостей), ComfyUI загрузит пакет без этих узлов и запишет предупреждение `[Arena] legacy disabled: …` в консоль. Остальные подсистемы Arena продолжат работать как обычно.
...
1. **Установите репозиторий целиком.** При установке через ComfyUI Manager всё содержимое попадает в `ComfyUI/custom_nodes/comfyui-arena-suite/`, и фронтенд автоматически ищет расширение в каталоге `web` этого пакета.
2. **Перезапустите ComfyUI.** После обновления репозитория перезапустите интерфейс, чтобы расширение `arena_autocache.js` инициализировалось.
...
4. **Проверьте загрузку и подсветку.** В браузере откройте DevTools → **Network** и убедитесь, что ресурс `extensions/arena_autocache.js` подгрузился без ошибок; сами ошибки ищите в консоли ComfyUI или браузера. После появления данных заголовок узла, полосы прогресса и подсказки обновятся автоматически.
...
- **Локализация.** Названия сокетов с любыми переводами (`Summary JSON`, `JSON сводки`, `Прогрев JSON`, `Обрезка JSON`) автоматически нормализуются к полям `summary`/`warmup`/`trim`, поэтому оверлей обновляется независимо от языка интерфейса.
